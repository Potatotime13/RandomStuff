    ...
    for d in data:
        if len(d['Price']) > 1:
            for i in range(len(d['Price'])):
                data_flat.append(
                    {'TIMESTAMP_UTC': d['TIMESTAMP_UTC'], 'Price': d['Price'][i], 'Volume': d['Volume'][i]})
        else:
            data_flat.append(
                {'TIMESTAMP_UTC': d['TIMESTAMP_UTC'], 'Price': d['Price'][0], 'Volume': d['Volume'][0]})
    return data_flat
...
def get_dirlist(book: bool):
    path_trades = u'C:/Users/Lucas/Downloads/archive/_shared_storage/' \
                  u'read_only/efn2_backtesting/trades/'
    path_book = u'C:/Users/Lucas/Downloads/archive/_shared_storage/' \
                u'read_only/efn2_backtesting/book/'
    dir_list_t = [unicodedata.normalize('NFC', f)
                  for f in os.listdir(path_trades)]
        ...
        for file_t, file_b in zip(files_t, files_b):
            ...
            if not trade is None:
                ...
                for i in range(trade.shape[0]):
                    ...
                    edges.loc[ind, 'Price'] = np.abs(
                        trade.loc[ind, 'Price']-mid[timestamp])
...
def create_volume_summary(stock_list, dir_list):
    ...
    for stock in tqdm(stock_list):
        ...
        for file in files:
            ...
            if len(data) > 0:
                data_df = pd.DataFrame(data)
                data_df['TIMESTAMP_UTC'] = pd.to_datetime(
                    data_df['TIMESTAMP_UTC'])
                data_df['hours'] = data_df['TIMESTAMP_UTC'].dt.hour
                data_df['Volume_agr'] = data_df.apply(
                    lambda row: sum(row['Volume']), axis=1)
                summary = data_df[['hours', 'Volume_agr']
                                  ].groupby(['hours']).sum()
                if hour_avg is None:
        ...
        for file in files:
            ...
            if len(data) > 0:
                ...
                data_df['TIMESTAMP_UTC'] = pd.to_datetime(
                    data_df['TIMESTAMP_UTC'])
                data_df['hours'] = data_df['TIMESTAMP_UTC'].dt.hour
                summary = data_df[['hours', 'Volume', 'Multiply']].groupby([
                                                                           'hours']).sum()
                ...
                for ind in range(summary.shape[0]):
                    ...
                    std_agr[ind] = ((data_df.loc[data_df['hours'] == hour, 'Price'] -
                                    summary.loc[hour, 'Price_avg'])**2).sum() / summary.loc[hour, 'Price_avg']
                if hour_avg is None:
                    hour_avg = std_agr
                else:
                    hour_avg += std_agr
